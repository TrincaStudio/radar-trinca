...
const PIPELINE_STAGES = ["Não contatado", "DM enviada", "Respondeu", "Reunião marcada", "Proposta enviada", "Cliente"];
...
const STORAGE_KEY = "radar-trinca:saude";
...
export default function RadarTrinca() {
      ...
      try {
        ...
        if (result) {
          ...
          if (Array.isArray(parsed) && parsed.length) {
            // Merge with current defaults so companies saved before newer fields
            // (pipeline, análise de produto, etc.) existed don't break the UI.
            setCompanies(parsed.map((c) => ({ ...BLANK_ANALYSIS_FIELDS, ...c })));
          }
        }
      } catch (e) {
  ...
  const stats = useMemo(() => {
    ...
    const porEstagio = Object.fromEntries(
      PIPELINE_STAGES.map((s) => [s, companies.filter((c) => c.estagioPipeline === s && !c.perdido).length])
    );
    const perdidos = companies.filter((c) => c.perdido).length;
    return { total, analisadas, verde, amarelo, vermelho, porEstagio, perdidos };
  }, [companies]);
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 20px 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0,1fr))", gap: 8, marginTop: 6 }}>
            {PIPELINE_STAGES.slice(1).map((stage) => (
              <Stat key={stage} label={stage} value={stats.porEstagio[stage]} color="#1CBFFF" />
            ))}
            <Stat label="Perdidos" value={stats.perdidos} color={CLASS_STYLES.Vermelho.text} />
          </div>
            ...
            {filtered.map((c) => (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  ...
                  {c.perdido ? (
                    <span className="mono" style={{ fontSize: 10.5, color: CLASS_STYLES.Vermelho.text }}>PERDIDO</span>
                  ) : (
                    c.estagioPipeline &&
                    c.estagioPipeline !== "Não contatado" && (
                      <span className="mono" style={{ fontSize: 10.5, color: "#1CBFFF" }}>{c.estagioPipeline.toUpperCase()}</span>
                    )
                  )}
...
// ---------------------------------------------------------------------------
function CompanyDrawer({ company, onClose, onUpdate }) {
              ...
              <div style={{ marginTop: 6, marginBottom: 6 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {PIPELINE_STAGES.map((stage, i) => {
                    const currentIdx = PIPELINE_STAGES.indexOf(company.estagioPipeline || "Não contatado");
                    const active = i === currentIdx;
                    const reached = i <= currentIdx;
                    return (
                      <button
                        key={stage}
                        onClick={() => onUpdate({ estagioPipeline: stage, perdido: false })}
                        style={{
                          padding: "6px 11px",
                          borderRadius: 999,
                          fontSize: 12,
                          fontWeight: 600,
                          border: "1px solid " + (active ? "#1CBFFF" : reached ? "#3a3733" : "#302d28"),
                          background: active ? "rgba(28,191,255,0.15)" : reached ? "#2c2925" : "transparent",
                          color: active ? "#7fd9ff" : reached ? "#c9c4bb" : "#65605a",
                          cursor: "pointer",
                        }}
                      >
                        {stage}
                ...
                <button
                  onClick={() => onUpdate({ perdido: !company.perdido })}
                  style={{
                    marginTop: 10,
                    display: "inline-flex",
                    alignItems: "center",
                    ...
                    fontSize: 11.5,
                    fontWeight: 600,
                    border: "1px solid " + (company.perdido ? "#FF3F85" : "#38352f"),
                    background: company.perdido ? "rgba(255,63,133,0.12)" : "transparent",
                    color: company.perdido ? "#ff6ba0" : "#8f8a80",
                    cursor: "pointer",
                  }}
                >
                  <AlertCircle size={12} /> {company.perdido ? "Marcado como perdido" : "Marcar como perdido"}
                </button>
              </div>
